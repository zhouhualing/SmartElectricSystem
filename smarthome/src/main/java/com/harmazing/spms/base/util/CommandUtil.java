...
public class CommandUtil {
	...
	private final static class Subscribe {
		...
		public final static class Patterns {
			...
			public final static String RESPONSE_IRCONTROL = "RESPONSE:IRCONTROL:*";
			public final static String RESPONSE_HALAMPCONTROL = "RESPONSE:HALAMPCONTROL:*";
			
			public final static String RESPONSE_RCUUPDATE = "RESPONSE:RCUUPDATE:*";
		...
		private static synchronized void init() {
			if (instance == null) {
				...
				try {
					jedis = new Jedis(
							PropertyUtil.getPropertyInfo("redis.server"),
							Integer.parseInt(PropertyUtil
									.getPropertyInfo("redis.port")), 2000);
					jedis.auth(PropertyUtil.getPropertyInfo("redis.password"));
				...
				while (true) {
					...
					try {
						jedis.psubscribe(subscribeListener, new String[] {
								...
								Patterns.RESPONSE_IRCONTROL,
								Patterns.RESPONSE_RCUUPDATE,
	...
	private final static class SubscribeListener extends JedisPubSub {
		...
		private class TimeoutTask extends TimerTask {
			...
			public void run() {
				...
				Iterator<String> iterator = subscribeListener.listeners
						.keySet().iterator();
				while (iterator.hasNext()) {
					String channel = iterator.next();
					ListenerRunable runable = subscribeListener
							.getListener(channel);
		...
		private JedisFactory() {
			...
			try {
				jedisPool = new JedisPool(config,
						PropertyUtil.getPropertyInfo("redis.server"),
						Integer.parseInt(PropertyUtil
								.getPropertyInfo("redis.port")),2000,PropertyUtil.getPropertyInfo("redis.password"));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		try {
			...
			if (messageType == null
					|| (!messageType.equals("COMMAND")
							...
							&& !messageType.equals("RCUUPDATE") //TODO
							)) {
	...
	public final static int syncSendMessage(Map message) throws IOException {
		...
		try {
			...
			if (messageType == null
					|| (!messageType.equals("COMMAND")
							&& !messageType.equals("IRCONTROL")
							&& !messageType.equals("HALAMPCONTROL")
							&& !messageType.equals("GETGWINFO") 
							&& !messageType.equals("GETSENSORINFO")
							&& !messageType.equals("SERVICEUPDATE")
							&& !messageType.equals("TIMERCURV")
	 ...
	 */
	public final static Map buildCommandMessageMap(String scope, String id,
			String sensorId, Integer deviceType,String commandType, String stringParameter,
			Integer intParameter) {
		if (scope == null
				|| (!Scope.AREA.equals(scope) && !Scope.USER.equals(scope) && !Scope.DEVICE
						.equals(scope))) {
	 ...
	 * @return
	 */
	public final static Map buildHALampControlMessageMap(String scope, String id,
			String sensorId, Integer[] irgb) {
		if (scope == null
				|| (!Scope.AREA.equals(scope) && !Scope.USER.equals(scope) && !Scope.DEVICE
						.equals(scope))) {
			throw new IllegalArgumentException("scope is not right!");
		...
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("messageType", "HALAMPCONTROL");
		map.put("scope", scope);
		map.put("id", id);
		map.put("sensorId", sensorId);
		map.put("illuminance", irgb[0]);
		map.put("red", irgb[1]);
		map.put("green", irgb[2]);
		map.put("blue", irgb[3]);
		return map;
	}
	 ...
	 * @return
	 */
	public final static Map buildRcUpdateMessageMap(String gwMac, String devMac, String rcuId) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("messageType", "RCUUPDATE");
		map.put("id", gwMac);
		map.put("sensorId", devMac);
		map.put("rcuId", rcuId);
		return map;
	}
