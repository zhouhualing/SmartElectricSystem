...
(function(m, ma) {
	...
	function Yb(a, b, e) {
		...
		function f() {
			...
			C = new Zb(X, b); (P = C.render()) && a.prepend(P);
			y(b.defaultView);
			m(window).resize(oa);
			t() || g()
		}
		function g() {
			setTimeout(function() { ! n.start && t() && S()
			},
			0)
		}
		function l() {
		...
		b.droppable && m(document).bind("dragstart",
		function(k, D) {
			var Z = k.target,
			ja = m(Z);
	...
	function Zb(a, b) {
		...
		function f(u) {
			var fa = m("<td class='fc-header-" + u + "'/>"); (u = b.header[u]) && m.each(u.split(" "),
			function(oa) {
	...
	function $b(a, b) {
		...
		function ga(c) {
			...
			for (var z = Aa.sourceNormalizers,
			H = 0; H < z.length; H++) z[H](c)
		}
		function ra(c, z) {
	...
	function Rb(a, b, e) {
		...
		function H(h) {
			if (h.which == 1 && i("selectable")) {
				...
				h);
				m(document).one("mouseup",
				function(x) {
					Ra.stop();
					if (O) { + O[0] == +O[1] && T(O[0], false, x);
						n(O[0], O[3], false, x)
					}
				})
			}
	...
	function Qb() {
		...
		function f(i) {
			...
			E, B;
			for (C = 0; C < P; C++) {
				E = i[C]; (B = E.element) && ha(E.event, B)
			}
		}
		function g(i, C, P) {
			var E, B = i.length,
	...
	function Mb() {
		...
		function d(Q) {
			...
			if (Q.which == 1 && g("selectable")) {
				...
				function(ra) {
					fa.stop();
					if (ga) { + ga[0] == +ga[1] && oa(ga[0], true, ra);
						e(ga[0], ga[1], true, ra)
					}
				})
			}
	...
	m.fn.fullCalendar = function(a) {
		...
		a = m.extend(true, {},
		Ya, a.isRTL || a.isRTL === ma && Ya.isRTL ? yc: {},
		a);
