...
function rSelects() {
	for (i=0;i<selects.length;i++){
		...
		select_tag = document.createElement('div');
			...
			select_tag.className = 'select_box2';
		selects[i].parentNode.insertBefore(select_tag,selects[i]);

		select_info = document.createElement('div');	
			select_info.id = 'select_info_' + selects[i].name;
			select_info.className='tag_select2';
			select_info.style.cursor='pointer';
		select_tag.appendChild(select_info);

		select_ul = document.createElement('ul');	
			select_ul.id = 'options_' + selects[i].name;
			select_ul.className = 'tag_options2';
...
function rOptions(i, name) {
	...
	for (n=0;n<selects[i].options.length;n++){	
		...
		if(option_selected){
			option_li.className='open_selected2';
			option_li.id='selected_' + name;
			getEleById('select_info_' + name).appendChild(document.createTextNode(option_li.innerHTML));
		}
		
		option_li.onmouseover = function(){	this.className='open_hover2';}
		option_li.onmouseout = function(){
			if(this.id=='selected_' + name){
				this.className='open_selected2';
			}
			else {
				this.className='open';
			}
...
function mouseSelects(name){
	...
	getEleById(sincn).onmouseover = function(){ if(this.className=='tag_select2') this.className='tag_select_hover2'; }
	getEleById(sincn).onmouseout = function(){ if(this.className=='tag_select_hover2') this.className='tag_select2'; }
...
function clickSelects(name){
	...
	for (i=0;i<selects.length;i++){	
		if(selects[i].name == name){				
			if( getEleById(sincn).className =='tag_select_hover2'){
				getEleById(sincn).className ='tag_select_open2';
				getEleById(sinul).style.display = '';
			}
			else if( getEleById(sincn).className =='tag_select_open2'){
				getEleById(sincn).className = 'tag_select_hover2';
				getEleById(sinul).style.display = 'none';
			}
		}
		else{
			getEleById('select_info_' + selects[i].name).className = 'tag_select2';
