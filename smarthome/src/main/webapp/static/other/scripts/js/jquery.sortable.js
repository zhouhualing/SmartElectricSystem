...
$.fn.sortable = function(options) {
	...
	return this.each(function() {
		...
		items.attr('draggable', 'true').bind('dragstart.h5s', function(e) {
			...
			if (items.is(this)) {
				dragging.hide();
				var that =placeholders.previousSbiling
				$(this)[placeholder.index() < $(this).index() ? 'after' : 'before'](placeholder);
				/*var moduleA =$(this).attr("module");
				var moduleB = $(placeholder).attr("module");
				console.log(moduleA)
				console.log(moduleB)
				if(placeholder.index()< $(this).index()){
					$(".block_charts li[module="+moduleA+"]").after($(".block_charts li[module="+moduleB+"]"));
				}else{
					$(".block_charts li[module="+moduleA+"]").before($(".block_charts li[module="+moduleB+"]"))
				}*/
				//$(this).hide();
