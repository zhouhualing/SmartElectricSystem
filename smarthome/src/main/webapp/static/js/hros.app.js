/**
 * boa='a':将应用从1位置拖动到2位置的时候，位移没有越过2位置原来应用的一半，则默认没越过2位置。
 * boa='b':将应用从1位置拖动到2位置的时候，无视位移是否越过2位置原来应用的一半，直接越过2位置。
 * 全文去掉boa，写死为‘b’，只要应用拖动的时候有覆盖，就默认拖动过去。
 */
	...
	return {
		...
		init : function(){
			...
			$('#desk').on('click', 'li.add', function(){
				HROS.window.createTemp({
					appid : 'hoorayos-yysc',
					title : '应用市场',
					url : htmlBase+'appstore/index.html',
					width : 800,
					height : 484,
					isflash : false
				});
		...
		*/
		get : function(){
			...
			$.ajax({
				url : ajaxUrl+"/findPCAllApps.json",
				type:'POST',
				data:{'userId':userId},
		        dataType: "json",
		        success: function(r) {
		            ...
		            HROS.VAR.dock = r['dock'];
					HROS.VAR.desk1 = r['desk1'];
					HROS.VAR.desk2 = r['desk2'];
					HROS.VAR.desk3 = r['desk3'];
					HROS.VAR.desk4 = r['desk4'];
					HROS.VAR.desk5 = r['desk5'];
					HROS.VAR.folder = r['folder'];
					//输出桌面应用
					HROS.app.set();
		        },
		        error: function(e) {
		...
		*/
		add : function(id, callback){
			...
			doRequest("/app/addMyApp",{id:id,desk:HROS.CONFIG.desk}, function (result, status) {
				 if (result.result) {
					 done();
		         }else{
		        	 alert('添加失败');
...
//			$.ajax({
//				url : ajaxUrl+"/addMyApp?id="+ id + '&desk=' + HROS.CONFIG.desk,
//				dataType: "text",
...
//			});
		...
		*/
		remove : function(id, callback){
			...
			doRequest("/app/delApp", {id:id,desk:HROS.CONFIG.desk,appPlace:appPlace,from:'PC'}, function (result, status) {
				 if (result.result) {
					 done();
		         }else{
		        	 alert('删除失败');
...
//				url : ajaxUrl+"/delApp",
//				dataType: "text",
//				data:{'id':id,'desk':HROS.CONFIG.desk,'appPlace':appPlace,'from':'PC'},
//				success : function(str){
//					done();
//				},
//				error:function(str){
						...
						if(movegrid != null){
							...
							if(movegrid != null){
								if(movegrid != oldobj.index()){
									...
									if(!HROS.app.checkIsMoving()){
										if(HROS.app.dataDockToDock(id, from, to, 'b')){
											HROS.VAR.isAppMoving = false;
											$.ajax({
												url : ajaxUrl+"/moveMyApp",
												data : 'movetype=dock-dock&id=' + id + '&from=' + from + '&to=' + to  ,
												success:function(){
													HROS.VAR.isAppMoving = false;
												}
											});
								...
								if(movegrid != null){
									...
									if(!HROS.app.checkIsMoving()){
										if(HROS.app.dataDockToDesk(id, from, to, 'b', desk)){
											$.ajax({
												url : ajaxUrl+"/moveMyApp",
												data : 'movetype=dock-desk&id=' + id + '&from=' + from + '&to=' + to +  '&desk=' + desk,
												success:function(){
													HROS.VAR.isAppMoving = false;
												}
											});
					...
					$(document).on('mousemove', function(e){
						...
						if(movegrid != null){
							...
							if(movegrid != null){
								...
								var desk = HROS.CONFIG.desk;
								if(!HROS.app.checkIsMoving()){
									if(HROS.app.dataDeskToDock(id, from, to, 'b', desk)){
										$.ajax({
											url : ajaxUrl+"/moveMyApp",
											data : 'movetype=desk-dock&id=' + id + '&from=' + from + '&to=' + to  + '&desk=' + desk,
											success:function(){
												HROS.VAR.isAppMoving = false;
											}
										});
								...
								if(movegrid != null && movegrid != oldobj.index()){
									...
									var desk = HROS.CONFIG.desk;
									if(!HROS.app.checkIsMoving()){
										if(HROS.app.dataDeskToDesk(id, from, to, 'b', desk)){
											$.ajax({
												url : ajaxUrl+"/moveMyApp",
												data : 'movetype=desk-desk&id=' + id + '&from=' + from + '&to=' + to  + '&desk=' + desk,
												success:function(){
													HROS.VAR.isAppMoving = false;
												}
											});
					...
					$(document).on('mousemove', function(e){
						...
						if(movegrid != null){
							...
							if(movegrid != null){
								...
								var desk = HROS.CONFIG.desk;
									if(!HROS.app.checkIsMoving()){
										if(HROS.app.dataFolderToDock(id, from, to, fromFolderId, 'b', desk)){
											$.ajax({
												type : 'POST',
												url : ajaxUrl,
												data : 'ac=moveMyApp&movetype=folder-dock&id=' + id + '&to=' + to  + '&desk=' + desk
											}).done(function(responseText){
												HROS.VAR.isAppMoving = false;
											});
										}
								...
								if(movegrid != null){
									...
									var desk = HROS.CONFIG.desk;
										if(!HROS.app.checkIsMoving()){
											if(HROS.app.dataFolderToDesk(id, from, to, fromFolderId, 'b', desk)){
												$.ajax({
													type : 'POST',
													url : ajaxUrl,
