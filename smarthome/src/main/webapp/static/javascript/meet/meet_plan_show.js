...
function addContentFun(obj,meetContent,id,mark,type,systemFiles) {
	...
	for(var i=0; i<attachmentFiles.length; i++) {
		attachemntStr = attachemntStr+ "<label><div><input type='hidden' name='attachmentId' value='"+attachmentFiles[i].id+"'/><a style='color:black;font-weight: normal;' class='print' href='"+attachmentFiles[i].url+"'><u>"+attachmentFiles[i].name+"</u></a></div></label>&nbsp;&nbsp;"
	}
    for(var i=0; i<nowSystemFiles.length; i++) {
        ...
        attachemntStr = attachemntStr+ "<label><div style='color:blue'><input type='hidden' name='contentDocId' value='"+nowSystemFiles[i].id+"'/><a  href='"+href+"' target='_blank'>"+nowSystemFiles[i].businessTitle+"</a></div></label>&nbsp;&nbsp;"
    }
