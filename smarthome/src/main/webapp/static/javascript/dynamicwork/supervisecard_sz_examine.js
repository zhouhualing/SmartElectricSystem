...
var btnTask = getURLParam("btnTask");
	...
	if(id != null) {
		...
		doJsonRequest("/super/getSuperVise",dto,function(data){
			...
			wf_getOperator(dto,function(data){
				if(btnTask!='' && btnTask!=null){
					$("#flow3 button").trigger("click");
				}
				...
				if(userTask == "usertask1"){
					...
					$("#remark").removeAttr("readOnly");
					//添加子流程按钮：
					$("#buttonList").append("<span><button type='button' class='btn btn_click' onclick='openNewFlow(\"Fsz\")'>副市长批示</button></span>&nbsp;<span><button type='button' class='btn btn_click' onclick='openNewFlow(\"Cbbm\")'>承办部门办理</button></span>");
				}else if(userTask == "usertask3" || userTask == "usertask5" ){
					//市长&副市长
					if($("#unitSpan").html()==''){
						$("#content").removeAttr("readOnly");
					}
...
function getAddData(){
	...
	if(userTask == "usertask3"){
		...
		//市长
		dto.szOpinion = makeOpinions($("#szOpinion").val(),$("#ldyj").val());
	}
	return dto;
}
...
function openNewFlow(task){
	var dto={
		id:id,
		content:$("#content").val(),
		remark:makeOpinions($("#remarkSpan").html(),$("#remark").val())
	}
	doJsonRequest("/super/addSecOpinion",dto,function(data){
		window.location.href="supervisecard_szBranch_draft.html?id="+id+"&mark="+mark+"&fromPage="+fromPage+"&btnTask="+task+"&taskId="+taskId+"&fromRoleCode="+$("#roleCode").val();
	},{showWaiting:true});
}
