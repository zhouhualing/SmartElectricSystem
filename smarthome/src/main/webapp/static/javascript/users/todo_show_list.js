/**
 * 分页插件 jquery_pagination 修改版 http://hooray.github.com/jquery.pagination/
 */
(function($) {
			...
			.extend(
					$.PaginationRenderers.defaultRenderer.prototype,
					{
						createLink : function(page_id, current_page, appendopts) {
							...
							if (page_id == current_page) {
								if (isNaN(appendopts.text)) {
									lnk = $("<li class='disabled'><a>"
											+ appendopts.text + "</a></li>")
								} else {
									lnk = $("<li class='active'><a>"
											+ appendopts.text + "</a></li>")
								}
							} else {
								lnk = $("<li><a href='"
						...
						getLinks : function(current_page, eventHandler) {
							...
							$("li:not(.disabled, .active) a", fragment).click(
									eventHandler);
							return fragment
						}
					});
	$.fn.pagination = function(maxentries, opts) {
		...
		function paginationClickHandler(evt) {
			var links, new_current_page = $(evt.target).parent()
					.data("page_id"), continuePropagation = selectPage(new_current_page);
