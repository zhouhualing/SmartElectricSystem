...
var ignoreRole = getURLParam("ignoreRole");
...
//		arr2 = clickmedTables.orgTargetDiv.selectObjs;
		var tempArr2 = zTree_Menu.getCheckedNodes()
		for(var i=0; i<tempArr2.length; i++) {
			if(!tempArr2[i].isParent) {
			...
			beforeCheck:function(id,node){
				...
				if(printType =="0002") {
					if(parentNode != null){
						...
						var nodes = zTree_Menu.getNodesByParam("parentTId", node.tId, null);
...
$(document).ready(function(){
	if(selectType=="0004" || selectType=="0005") {
		treeObj = $("#treeDemo");
		...
		doJsonRequest("/workflow/initWorkFlowGroup",dto,function(data,status){
			if(data.result) {
				var data = data.data;
				var ignoreRoles = ignoreRole.split(",");
				var theData = new Array();
				for(var i=0; i<data.length; i++) {
					if($.inArray(data[i].code,ignoreRoles) ==-1) {
						var level1Data = $.extend(true,{},data[i]);
						level1Data.children=null;
						var theCData = new Array();
						level1Data.children = theCData;
						for(var j=0;j<data[i].children.length;j++) {
							if($.inArray(data[i].children[j].code,ignoreRoles) ==-1) {
								theCData.push(data[i].children[j]);
							}
						}
						if(theCData.length > 0) {
							theData.push(level1Data);
						}
						clickFlag = level1Data.open;
					} 
				}
				zTree_Menu = $.fn.zTree.init(treeObj, setting, theData);
				if(!clickFlag) {
